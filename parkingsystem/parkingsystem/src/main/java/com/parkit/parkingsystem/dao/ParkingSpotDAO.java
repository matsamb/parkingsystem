...
public class ParkingSpotDAO {
	...
	public boolean updateToEmptyParking() {
		...
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.UPDATE_EMPTY_PARKING_SPOT_TEST);
			int updateAvailability = ps.executeUpdate();
			dataBaseConfig.closePreparedStatement(ps);
	...
	public boolean updateToFullParking() {
		// update the availability for that parking slot
		Connection con = null;
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.UPDATE_FULL_PARKING_SPOT_TEST);
			int updateAvailability = ps.executeUpdate();
			dataBaseConfig.closePreparedStatement(ps);
			return (updateAvailability == 1);
		} catch (Exception ex) {
	...
	//à faire
	public boolean updateToFullParkingWithSameReg(String vehicleRegNumber) {
		// update the availability for that parking slot
		Connection con = null;
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.UPDATE_FULL_PARKING_SPOT_WITH_SAME_REG_TEST);
